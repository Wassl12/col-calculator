...
def get_all_taxes(salary: int, state: str, exemptions: int, city: City = City.OTHER):
    ...
    return round(
        calculate_federal_income_tax(salary)
        + calculate_state_income_tax(salary, State(state.lower()), exemptions)
        + calculate_fica(salary)
        + calculate_local_income_tax(salary, city)
    )
